...
uint8_t BM1373::init(uint64_t frequency, uint16_t asic_count, uint32_t difficulty, uint32_t vrFrequency)
{
    ...
    // [TEST A] BM1373: 0xB9 register with all bits set
    // Hypothesis: this register enables additional core groups.
    // Original BM1370 value: 0x00 0x00 0x44 0x80 (only 3 bits set across 16)
    // Test A: 0x00 0x00 0xFF 0xFF (all bits set)
    send6(CMD_WRITE_ALL, 0x00, 0xB9, 0x00, 0x00, 0xFF, 0xFF);

    // Analog Mux Control
    send6(CMD_WRITE_ALL, 0x00, 0x54, 0x00, 0x00, 0x00, 0x02);

    // [TEST A] BM1373: 0xB9 second write (also with all bits set)
    send6(CMD_WRITE_ALL, 0x00, 0xB9, 0x00, 0x00, 0xFF, 0xFF);
